            ...
            if ($useTorrent)
            {
                ...
                if ($status['status'])
                {
                    ...
                    if (class_exists('EventBus'))
                        EventBus::emit(EventBus::EVENT_DOWNLOAD_START, ['id' => $id, 'name' => $name, 'tracker' => $tracker]);
                }
                else
                {
                    $message = $message.' Но не добавлен в torrent-клиент и сохранён.';
                    if (class_exists('EventBus'))
                        EventBus::emit(EventBus::EVENT_DOWNLOAD_ERROR, ['id' => $id, 'name' => $name, 'tracker' => $tracker]);
                }
