<?php
// Запуск проверки состояния в фоновом процессе
...
if (file_exists($pidFile))
{
    ...
    {
        echo json_encode(['error' => false, 'msg' => 'Проверка уже выполняется']);
        exit;
    }
    @unlink($pidFile);
}
...
$workerPath = escapeshellarg(dirname(__FILE__).'/health_worker.php');
...
if ($pid > 0 && isProcessRunning($pid))
    echo json_encode(['error' => false, 'msg' => 'Проверка запущена', 'pid' => $pid]);
else
    echo json_encode(['error' => true, 'msg' => 'Не удалось запустить проверку']);
